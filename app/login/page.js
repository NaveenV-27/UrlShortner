...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push("/");
        router.refresh(); // Forces Next.js to re-check server components for auth status
      } else {
  ...
  return (
    <main className="min-h-[90vh] flex items-center justify-center bg-gradient-to-br from-cyan-50 to-cyan-100">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-4">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-cyan-600 font-serif">Welcome Back</h1>
          <p className="text-gray-500 mt-2">Sign in to your BitLinks account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between text-sm">
            <label className="flex items-center text-gray-600">
              <input type="checkbox" className="mr-2 accent-cyan-600" />
              Remember me
            </label>
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600">
            ...
            <Link href="/signup" className="text-cyan-600 font-semibold hover:text-cyan-700 hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
